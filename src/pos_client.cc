...
CPositionClient::CPositionClient(const char *serverIp, int port,
...
CPositionClient::~CPositionClient() { s.close(); }
  ...
  if (s.available() > 0) {
    ...
    if (r > 0) {
      ...
      if (strcmp((const char *)buffer, DATA_HEADER) == 0) {
        ...
        // for (int i=0; i<r; i++) { printf(" %d (%c)", (int)buffer[i],
        // buffer[i]); } printf("\n");
        CPositionMessage *pos =
            (CPositionMessage *)(buffer + sizeof(DATA_HEADER));
        if (data_callback) {
      ...
      } else if (strcmp((const char *)buffer, ECHO_MESSAGE_HEADER) == 0) {
        uint32_t t = getTimeMs();
        CPingMessage *echo =
            (CPingMessage *)(buffer + sizeof(ECHO_MESSAGE_HEADER));
