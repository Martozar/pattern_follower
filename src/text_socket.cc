...
const char *socket_error_str[] = {"OK",
                                  "SOCKET_ERROR_CREATE",
                                  "SOCKET_ERROR_BIND",
                                  "SOCKET_ERROR_LISTEN",
                                  "SOCKET_ERROR_ACCEPT",
...
void *text_socket_thread(void *param) {
  return ((CTextSocket *)param)->threadBody();
}

CTextSocket::CTextSocket() : thread(text_socket_thread, this) {
  sockfd = -1;
...
bool CTextSocket::isConnected() { return (sockfd >= 0); }
...
bool CTextSocket::send(uint8_t *data, int length) {
  ...
  try {
    ...
    if (wb == -1) {
      text_socket_debug("ERROR: write on CTextSocket(%d) returned errno %s",
      ...
      text_socket_debug("CTextSocket (%d) send message length=%d, sent=%d",
                        sockfd, len, wb);
    }
  } catch (int e) {
    wb = -1;
    text_socket_debug(
        "EXCEPTION %d catched on CTextSocket(%d)/write(), errno=%s", e, sockfd,
        strerror(errno));
  }
  return (wb == len);
}
...
int CTextSocket::available() { return receive_length; }
...
void CTextSocket::shutdown() { stop(); }
...
void *CTextSocket::threadBody() {
  ...
  if (sigmask_ret != 0) {
    text_socket_info(
        "CTextSocket (%d) pthread_sigmask() returned with error %d", sockfd,
        sigmask_ret);
  }
  ...
  while (!q) {
    ...
    if (rb == 1) {
      // text_socket_debug("CTextSocket (%d) received byte %d (%02X)", sockfd,
      // buf[0], buf[0]);
      uint8_t c = buf[0];
      ...
      if (last != SLIP_ESC) {
        if (c == SLIP_END) {
          if (receive_length > 0) {
            text_socket_debug(
                "CTextSocket received message (len = %d bytes, cb=%c)",
                receive_length, (receive_callback_function) ? 'Y' : 'N');
      ...
      if (data_valid) {
        if (receive_length < TEXT_SOCKET_MAX_MESSAGE_SIZE) {
          ...
          if (!supress_buffer_warning) {
            text_socket_debug(
                "ERROR: CTextSocket (%d) message length exceeds buffer size",
                sockfd);
          }
        }
      }
      ...
      // READ ERROR
      text_socket_info("CTextSocket (%d) receive - read error %d (errno=%s)",
                       sockfd, rb, strerror(errno));
      q = true;
    }
    {
      // ScopedLock lk(mtx);
  ...
  if (disconnect_callback_function)
    disconnect_callback_function(this);
  return 0;
}
...
bool CServerTextSocket::waitForClient() {
  ...
  if (newsockfd < 0) {
    text_socket_debug(
        "CServerTextSocket::waitForClient() error (accept, errorcode=%d/%s)",
        errno, strerror(errno));
    last_error = SOCKET_ERROR_ACCEPT;
    return false;
  }
  text_socket_debug("CServerTextSocket client accepted, new socket fd=%d",
                    newsockfd);
  ...
  text_socket_debug(
      "CServerTextSocket client accepted, address=%s",
      print_ip_address(tmp, (uint8_t *)&cli_addr.sin_addr.s_addr));
...
bool CClientTextSocket::open(uint8_t *server_address, uint16_t server_port) {
  ...
  if (server_address) {
    ...
    server_addr.sin_family = AF_INET;
    bcopy((char *)server_address, (char *)&server_addr.sin_addr.s_addr,
          INET_ADDRESS_LENGTH);
  ...
  if (connect(sockfd, (sockaddr *)&server_addr, sizeof(server_addr)) < 0) {
    ...
    text_socket_debug("socket connect(fd=%d, addr=%d.%d.%d.%d:%d) failed.",
                      sockfd, a[0], a[1], a[2], a[3],
...
bool CClientTextSocket::reopen() { return open(NULL, 0); }
