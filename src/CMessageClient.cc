...
int CMessageClient::sendMessage(CMessage &message) {
  // dump->Debug(module,"Message sending %s %i
  // %i.",message->getStrType(),message->value1,message->value2);
  message.pack();
  if (send(mySocket, message.buf, MESSAGE_LENGTH, MSG_NOSIGNAL) ==
      MESSAGE_LENGTH)
    return 0;
  else {
...
int CMessageClient::init(const char *ip, const int port, bool requirements[]) {
  ...
  if (mySocket > 0) {
    ...
    if (host_info != NULL) {
      ...
      result = connect(mySocket, (struct sockaddr *)&server_addr,
                       sizeof(server_addr));
    }
    if (result == 0) {
      if (send(mySocket, requirements, 4 * sizeof(bool), MSG_NOSIGNAL) ==
          4 * sizeof(bool))
        return 0;
      else {
        //		    dump->Error(module,"Network error when sending
        //info");
        return -1;
      }
...
int CMessageClient::checkForInts(int data[], unsigned int len) {
  ...
  if ((unsigned int)lengthReceived == len * sizeof(int))
    result = 0;
  return result;
}
...
int CMessageClient::checkForBools(bool data[], unsigned int len) {
  ...
  if (lengthReceived == (int)(len * sizeof(bool)))
    result = 0;
  return result;
}
...
int CMessageClient::checkForDoubles(double data[], unsigned int len) {
  int result;
  int lengthReceived =
      recv(mySocket, data, len * sizeof(double), NETWORK_BLOCK);
  if (lengthReceived == (int)(len * sizeof(double)))
    result = 0;
  return result;
}
...
int CMessageClient::checkForStatus(CStatusMessage &status) {
  int result;
  int lengthReceived = recv(mySocket, status.getSerializedData(),
                            status.getExpectedLength(), NETWORK_BLOCK);

  // printf("CMessageClient::recv %d:", lengthReceived);
  // for (int i=0; i<lengthReceived; i++) printf(" %02X",
  // (int)(status.getSerializedData()[i])); printf("\n");
