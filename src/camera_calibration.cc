...
static void help() {
  cout
      << "This is a camera calibration sample." << endl
...
int run_calibration(cv::String argv) {
  ...
  for (int i = 0;; ++i) {
    ...
    if (mode == CAPTURING && imagePoints.size() >= (unsigned)s.nrFrames) {
      ...
      fprintf(stderr,
              "Running calibration in capturing mode, got enough (%d) frames\n",
              s.nrFrames);
    ...
    case Settings::CHESSBOARD: {
      ...
      if (scale > 1) {
        // Mat small = cvCreateImage( cvSize(imageSize.width / scale,
        // imageSize.height / scale), view.depth, view.channels );
        Mat small;
        // cvResize( view, small, CV_INTER_CUBIC );
        fprintf(stderr, "resizing input image by scale %d\n", scale);
        resize(view, small,
               Size(imageSize.width / scale, imageSize.height / scale), 0, 0,
               INTER_CUBIC);
        found = findChessboardCorners(small, s.boardSize, pointBuf,
      ...
      if (!found) {
        fprintf(stderr,
                "Invalid file: can not find chessboard corner in image %s\n",
    ...
    if (found) // If done with success,
    {
      ...
      if (s.calibrationPattern == Settings::CHESSBOARD) {
        ...
        cornerSubPix(viewGray, pointBuf,
                     Size(5 * scale, 5 * scale), // original was (11, 11)
                     Size(-1, -1),
...
static bool runCalibration(Settings &s, Size &imageSize, Mat &cameraMatrix,
  ...
  for (int i = 0; i < distCoeffs.rows; i++)
    fprintf(stderr, " %.5f", distCoeffs.at<double>(i, 1));
  fprintf(stderr, "\n");
