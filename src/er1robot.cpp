	...
	while(1) {
		...
		if (motionEnabled) {
			if (!velocityControlMode) {
				if (path!=NULL && pathLength>0 && pathPointer>=0 && pathPointer<pathLength) {
					//printf("task %d\n", pathPointer);
			...
			//printf("path=%p  len=%d  ptr=%d  mot=%d\n", path, pathLength, pathPointer, motionEnabled);
		}
...
bool CER1Robot::setRelativeVelocities(double forwardVelocity, double angularVelocity)
{
   pthread_mutex_lock(&er1_lock);
	velocityControlMode = true;
	leftWheelVelocity = GO_SPEED * forwardVelocity - TURN_SPEED * angularVelocity;
	rightWheelVelocity = GO_SPEED * forwardVelocity + TURN_SPEED * angularVelocity;
   pthread_mutex_unlock(&er1_lock);
}
...
bool CER1Robot::moveLine(CTask* task)
{
	...
	if (!task->initialized){
		...
		task->tresholdLine[2] = -(task->tresholdLine[0]*(x+task->length*cos(h)) + task->tresholdLine[1]*(y+task->length*sin(h)));
		printf("DEBUG: LINE len=%.3f a=%.3f b=%.3f c=%.3f\n", task->length, task->tresholdLine[0], task->tresholdLine[1], task->tresholdLine[2]);
		...
		double t=(task->tresholdLine[0]*x+task->tresholdLine[1]*y+task->tresholdLine[2]);
		printf("DEBUG: t=%.3f\n", t);
		if (t>0-correction){
			rcm.setVelocities(0.0, 0.0, true);
			result = true;
