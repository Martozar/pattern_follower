...
public:
  ...
  int receive(char *str, int max_length) {
    return receive((uint8_t *)str, max_length);
  }
  int available(); // WARNING: check available only in receive callback function
  void setReceiveCallback(void (*fce)(CTextSocket *)) {
    receive_callback_function = fce;
  }
  void setAcceptCallback(void (*fce)(CTextSocket *)) {
    accept_callback_function = fce;
  }
  void setDisconnectCallback(void (*fce)(CTextSocket *)) {
    disconnect_callback_function = fce;
  }
  ...
  void getAddress(uint8_t *address) {
    memcpy(address, &server_addr.sin_addr.s_addr, 4);
  }
